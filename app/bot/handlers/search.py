...
def result_label(result: SearchResult) -> str:
    ...
    return f"{icon} {result.title}" + (f" ({details})" if details else "")


def results_keyboard(results: list[SearchResult]) -> InlineKeyboardMarkup:
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"title:{result.media_type}:{result.tmdb_id}",
                )
            ]
...
def seasons_keyboard(seasons: list[Season]) -> InlineKeyboardMarkup:
    """Build a compact two-column season picker with a cancel action."""
    buttons = [
        InlineKeyboardButton(
            text=f"Season {season.number}", callback_data=f"season:{season.number}"
        )
        for season in seasons[:MAX_NAVIGATION_BUTTONS]
    ]
    rows = [buttons[index : index + 2] for index in range(0, len(buttons), 2)]
    rows.append([InlineKeyboardButton(text="Cancel", callback_data="nav:cancel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def episodes_keyboard(season_number: int, episodes: list[Episode]) -> InlineKeyboardMarkup:
    """Build a compact episode picker with back and cancel actions."""
    buttons = [
        InlineKeyboardButton(
            text=f"Episode {episode.number}",
            callback_data=f"episode:{season_number}:{episode.number}",
        )
...
async def title_search(message: Message) -> None:
    ...
    if not results:
        ...
        return
    conversation_store.set_search_results(message.from_user.id, results)
    logger.info(
        "title_search_completed",
        user_ref=user_reference(message.from_user.id),
        result_count=len(results),
    )
    await message.answer("Which title did you mean?", reply_markup=results_keyboard(results))
...
async def title_selected(callback: CallbackQuery) -> None:
    ...
    result = conversation_store.select_result(
        callback.from_user.id, MediaType(parts[1]), int(parts[2])
    )
    if result is None:
    ...
    if result.media_type is MediaType.MOVIE:
        await callback.message.answer(f"Selected: {result_label(result)}")
    ...
    if not seasons:
        ...
        return
    conversation_store.set_seasons(callback.from_user.id, seasons)
    await callback.message.answer(
        "Choose a season:\nSpecials (Season 0) are not selectable.",
        reply_markup=seasons_keyboard(seasons),
    )
...
async def season_selected(callback: CallbackQuery) -> None:
    ...
    if conversation_store.select_season(callback.from_user.id, season_number) is None:
    ...
    if not episodes:
        ...
        return
    conversation_store.set_episodes(callback.from_user.id, episodes)
    await callback.answer()
    await callback.message.answer(
        "Choose an episode:", reply_markup=episodes_keyboard(season_number, episodes)
    )
...
async def episode_selected(callback: CallbackQuery) -> None:
    ...
    if len(parts) != 3 or not parts[1].isdigit() or not parts[2].isdigit():
        ...
        return
    episode = conversation_store.select_episode(callback.from_user.id, int(parts[1]), int(parts[2]))
    if episode is None:
        await callback.answer("That episode has expired. Search again.", show_alert=True)
        return
    await callback.answer()
    if callback.message is not None:
        await callback.message.answer(f"Selected episode: {episode.name}")
        await search_subtitles(callback.message, callback.from_user.id)


@router.callback_query(F.data.in_({"nav:back", "nav:cancel"}))
async def navigation_action(callback: CallbackQuery) -> None:
    """Handle safe in-keyboard back and cancel actions."""
    if callback.from_user is None:
        await callback.answer()
        return
    if callback.data == "nav:cancel":
        ...
        return
    if not conversation_store.back_to_seasons(callback.from_user.id):
        await callback.answer("Those seasons have expired. Search again.", show_alert=True)
        return
    seasons = list(conversation_store.get(callback.from_user.id).seasons.values())
    await callback.answer()
    if callback.message is not None:
        await callback.message.answer("Choose a season:", reply_markup=seasons_keyboard(seasons))
